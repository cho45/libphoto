...
var distances = [150, 200, 250, 300, 500, 1000];
var acuities = [0.3, 0.5, 0.7, 1.0, 1.5, 2.0];

console.log('<table>');
console.log('<thead><tr><th></th>');
for (var j = 0, lenn = acuities.length; j < lenn; j++) {
	console.log('<th>', acuities[j].toFixed(1), '</th>');
}
console.log('</tr></thead><tbody>');

for (var i = 0, len = distances.length; i < len; i++) {
	console.log('<tr><th>', distances[i] / 10, ' cm</th>');
	for (var j = 0, lenn = acuities.length; j < lenn; j++) {
		var ppi = libphoto.dpiByDistanceAndVisualAcuity(distances[i], acuities[j]);
		console.log('<td>', ppi.toFixed(0), 'ppi</td>');
	}
	console.log('</tr>');
}
