...
function depthOfField (distance, focalLength, fNumber, circle) {
	...
	var distance1 = (hyperfocalDistance * distance) / (hyperfocalDistance + (distance - focalLength));
	var distance2 = (hyperfocalDistance * distance) / (hyperfocalDistance - (distance - focalLength));
	return [ distance1, distance2 > 0 ? distance2 : (1/0)];
}

/*
 * output サイズ, dpi, sensorSize から許容錯乱円を求める
 */
function circleOfConfusion (output, dpi, sensorSize) {
	return sensorSize / (
		output /
			(25.4 / dpi)
	);
...
this.rayleighLimit = rayleighLimit;
